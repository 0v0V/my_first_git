...
class MyQueue():
    ...
    def push(self, in_value):
        if isinstance(in_value, int):
            if in_value in self.filtered_queue:
                x = "error@duplicated"
                return x
            self.queue.append(in_value)
            self.num = in_value
            self.filtered_queue.append(self.num)
            self.dict.setdefault(self.num,in_value)
        elif isinstance(in_value, float):
            self.queue.append(in_value)
            self.num = int(in_value)
            self.filtered_queue.append(self.num)
            self.dict.setdefault(self.num,in_value)
            ...
            for i in in_value:
                if i.isdigit() == True:
                    self.n = self.n*10 + int(i)
            self.num = self.n
            self.filtered_queue.append(self.num)
            self.dict.setdefault(f"{self.num}",in_value)     
    ...
    def get_filtered_sorted(self):
        sorted_queue = sorted(self.filtered_queue)
        return sorted_queue
        
    def get_filtered_reversed(self):
        return self.filtered_queue[::-1]

    def pop(self, n="123"):
        b = self.dict.pop(f"{n}")
        self.filtered_queue.pop(self.queue.index(b))
        return {b : n}
    ...
    def __add__(self, other):
        if isinstance(other, MyQueue):
            result = MyQueue()
            for j in self.filtered_queue:
                result.push(j)
            for i in other.filtered_queue:
                result.push(i)
        return result
