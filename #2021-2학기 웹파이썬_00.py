...
class MyInteger():
    ...
    def __init__(self,n):
        if isinstance(n, int):
            self.n = n
        elif isinstance(n, str) or isinstance(n,float):
            self.n = int(n)
        else:
            self.n = 0
    ...
    def __add__(self,other):
        if isinstance(other,MyInteger):
            result = MyInteger(self.n + other.n)
        else:
            converOther = MyInteger(other)
            result = MyInteger(self.n +converOther.n)
        return result

    
    def _get_MyInteger(self):
        return MyInteger(self.n)
...
class MyDerivedInteger(MyInteger):
    ...
    def __str__(self):
        x = f"<{self.n}:{type(self.n)},{self.original},{type(self.original)}>"
        return x

    def __add__(self, other):
        if isinstance(other, MyInteger) or isinstance(other,MyDerivedInteger):
            result = MyDerivedInteger(self.n + other.n)
        else:
            converOther = MyDerivedInteger(other)
            result = MyDerivedInteger(self.n + converOther.n)
        return result

    def _get_MyInteger(self):
        return MyDerivedInteger(self.n)
